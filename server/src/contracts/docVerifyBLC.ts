...
export const storeDocumentHash = async (documentId: string, hash: string) => {
    try {
        // Convert the hash to bytes32 format - Fix for ethers v6
        const bytes32Hash = ethers.zeroPadValue(ethers.hexlify(`0x${hash}`), 32);
...
export const compareHashes = async (documentId: string, currentHash: string) => {
    try {
        ...
        // Extract the actual hash value from the bytes32 response - Fix for ethers v6
        const storedHashHex = ethers.dataSlice(storedHashBytes32, 0, 32).substring(2);
