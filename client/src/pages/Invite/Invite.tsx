  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      axios
        ...
        .then((res) => {
          setStatus("success");
          if (res.data.data === "hod") {
            setTimeout(() => window.location.href = "/hod", 1000);
          } else if (res.data.data === "project-manager") {
            setTimeout(() => window.location.href = "/pm", 1000);
          }
        })
        .catch((error) => {
          setStatus("error");
          setErrorMessage(error.response?.data?.message || "There was an error processing your invitation");
        });
